...
{
	global $wsRead, $wsClientCount, $wsClientIPCount, $repartirCarta, $nextFold, $revisando, $finalizado;
	...
	while (isset($wsRead[0])) {
		...
		/********************************** ICEON **********************************/
		if ($repartirCarta && !$revisando && !$finalizado) {
			if (time() >= $nextFold) {
				if ($nextFold == 0) {
					avisar();
					sleep(3);
				}
				//echo "Se reparte carta" . PHP_EOL;
				repartir();
				$nextFold = time() + 3;
			}
		}
